...
class DataLoader():
    ...
    def process_actions(self):
        """We add the actions to self.tokenized data"""
        print("Tokenizing text...")
        for i, one_action_data in enumerate(self.data):
            action = one_action_data["action"]
            self.tokenized_data[i]["action"] = action
    ...
    def build_batch(self, data):
        ...
        for i in range(len(data)):
            ...
            action_targets.append(torch.FloatTensor([self.action_to_id(data[i]["action"])]).unsqueeze(0))
        features = torch.cat(features, dim=0) 
